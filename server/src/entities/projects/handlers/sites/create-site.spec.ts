...
describe('createSite', () => {
  ...
  it('should create a site', async () => {
    ...
    const response = await request(meliServer.app)
      .post('/api/v1/projects/project-id/sites')
  ...
  it('should check if the project exists', async () => {
    ...
    expect(projects.countDocuments).toHaveBeenCalledWith({ _id: 'project-id' }, expect.anything());
  });


  it('should check if the user can administrate the project', async () => {
    const projects = spyOnCollection('Projects', {
      countDocuments: jest.fn().mockReturnValue(Promise.resolve(1)),
      findOne: jest.fn().mockReturnValue(Promise.resolve({ orgId: 'organization-id' })),
    });
