...
describe('createTeam', () => {
  ...
  it('should check that the user is owner or admin', async () => {
    const orgs = spyOnCollection('Orgs', {
      countDocuments: jest.fn().mockImplementation(async query => query.ownerId !== undefined ? 0 : 1),
    });
    const members = spyOnCollection('Members', {
      countDocuments: jest.fn().mockReturnValue(Promise.resolve(0)),
    });
    ...
    expect(orgs.countDocuments).toHaveBeenCalledTimes(2);
    expect(members.countDocuments).toHaveBeenCalled();
  });
